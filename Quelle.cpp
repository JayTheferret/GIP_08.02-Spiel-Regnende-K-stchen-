...
bool update_boxes(Box boxes[])
{
	...
	for (int i = 0; i < box_max; i++) {
		...
		if ((boxes[i].y + box_size) > gip_win_size_y)
			return(false);
...
void handle_mouse_click(Box boxes[])
{
	...
	// TO DO
	for (int i = 0; i < box_max; i++) {
		if (boxes[i].y <= mouse_y &&
			boxes[i].y + box_size >= mouse_y &&

			boxes[i].x <= mouse_x &&
			boxes[i].x + box_size >= mouse_x)
		{

			boxes[i].delta_y += 10;
			boxes[i].y = -boxes[i].delta_y; //? > rücksetzen nach oben
